  ...
  class _ProfilePageState extends State<ProfilePage> {
    ...
    Future<void> fetchUserData() async {
      ...
      if (user != null) {
        ...
        if (userDoc.exists) {
          setState(() {
            userName = userDoc['name']; // Assuming 'name' is a field in the 'users' document
          });
        }
      }
    }
    ...
    Future<void> fetchFavorites() async {
      User? user = FirebaseAuth.instance.currentUser;
      if (user != null) {
        QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('favorites').doc(user.uid).collection('classes').get();
        setState(() {
          favoriteClasses = snapshot.docs.map((doc) => doc['code'] as String).toList();
        });
      }
    }
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            if (favoriteClasses.isNotEmpty) 
              Column(
                children: favoriteClasses.map((classCode) {
                  return ListTile(
